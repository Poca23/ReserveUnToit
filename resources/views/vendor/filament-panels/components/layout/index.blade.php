...
<x-filament-panels::layout.base :livewire="$livewire">
    ...
    >
        ...
        @if (filament()->hasNavigation())
            ...
            {{-- Bouton pour FERMER la sidebar quand elle est ouverte --}}
            <div 
                x-data="{}"
                x-show="$store.sidebar.isOpen"
                x-transition.opacity.300ms
                class="fixed top-4 right-4 z-40 lg:hidden"
            >
                <button
                    type="button"
                    class="flex h-8 w-8 items-center justify-center rounded-full bg-orange-500 text-white shadow-lg ring-0 transition hover:bg-orange-400 focus:outline-none"
                    x-on:click="$store.sidebar.close()"
